...
| --- | --- | --- | --- | --- | --- | --- |
| C1 | Category 1 Type Safety baseline + measurable improvement | Met | `api/src/**`, `web/src/**`, `shared/src/**` | `PHASE_1_AUDIT_REPORT.md`, `PHASE_2_PRD_BUNDLE/evidence/type-check-remediation.log`, `PHASE_2_PRD_BUNDLE/evidence/open-set-remediation-closeout.md` | `pnpm type-check` + focused remediation validation pass | `pnpm test`, `pnpm test:e2e` |
| C2 | Category 2 Bundle Size baseline + measurable improvement | Met | `web/vite.config.ts`, lazy-loaded page/components in `web/src/**` | `PHASE_1_AUDIT_REPORT.md`, `PHASE_2_PRD_BUNDLE/evidence/build-closeout.log`, `PHASE_2_PRD_BUNDLE/evidence/c2-after-summary.md` | `pnpm --filter @ship/web exec tsc` + `pnpm --filter @ship/web exec vite build` | `pnpm test`, targeted web tests |
| C3 | Category 3 API latency baseline + measurable improvement | Met | API route/query changes in `api/src/routes/**`, `api/src/db/**`, `api/src/app.ts` | `PHASE_1_AUDIT_REPORT.md`, `PHASE_2_PRD_BUNDLE/evidence/c3-remediation-summary.md`, `PHASE_2_PRD_BUNDLE/evidence/c3-remed-*-c25.json`, `PHASE_2_PRD_BUNDLE/evidence/open-set-remediation-closeout.md` | autocannon benchmark loop at c25 with authenticated session | API route tests + E2E flows |
| C4 | Category 4 DB query efficiency baseline + measurable improvement | Met | `api/src/app.ts`, `api/src/db/**`, route query refactors | `PHASE_1_AUDIT_REPORT.md`, `PHASE_2_PRD_BUNDLE/evidence/c4-query-count-remediation.log`, `PHASE_2_PRD_BUNDLE/evidence/open-set-remediation-closeout.md` | metrics-enabled flow replay with query counter | API tests for affected routes |
| C5 | Category 5 test coverage/quality baseline + improvements | Met | `e2e/**`, `api/src/**/*.test.ts`, `web/src/**/*.test.tsx` | `PHASE_1_AUDIT_REPORT.md`, `PHASE_2_PRD_BUNDLE/evidence/api-test-after.log`, `PHASE_2_PRD_BUNDLE/evidence/test-coverage-after.log` | `pnpm test`, `pnpm test:e2e`, `pnpm test:coverage` | full test suites |
| C6 | Category 6 runtime error/edge-case handling improvements | Met | `api/src/app.ts`, `api/src/collaboration/index.ts`, UI runtime handlers | `PHASE_1_AUDIT_REPORT.md`, `PHASE_2_PRD_BUNDLE/evidence/c6-runtime-remediation.log`, `PRD_CAT8/security-probe-closeout-4.md`, `PRD_CAT8/error-leakage-check.log`, `PHASE_2_PRD_BUNDLE/evidence/open-set-remediation-closeout.md` | targeted repro scripts + malformed-input replay + probe rerun | `e2e/error-handling.spec.ts`, collaboration tests |
...
| Requirement ID | Requirement | Status | Implementation Evidence | Proof Artifact(s) | Reproduction Command(s) | Regression Test(s) |
| --- | --- | --- | --- | --- | --- | --- |
| C8-T1 | Runnable security probe tool (single command) | Met | `api/src/scripts/security-probe.ts`, root/api package scripts | `PRD_CAT8/security-probe-closeout.log` | `pnpm security:probe` | `e2e/security.spec.ts` |
| C8-T2 | Probe covers auth/session surface | Met | `api/src/scripts/security-probe.ts` auth probe functions | `PRD_CAT8/security-probe-closeout.json`, `PRD_CAT8/security-probe-closeout.md` | `pnpm security:probe` | auth + security tests |
| C8-T3 | Probe covers websocket validation surface | Met | `api/src/scripts/security-probe.ts` websocket probes | `PRD_CAT8/security-probe-closeout-3.json`, `PRD_CAT8/security-probe-closeout-3.md` | `pnpm security:probe` | websocket/collab tests |
| C8-T4 | Probe covers input sanitization surface | Met | `api/src/scripts/security-probe.ts` input probes | `PRD_CAT8/security-probe-closeout-3.json`, `PRD_CAT8/security-probe-closeout-3.md` | `pnpm security:probe` | route tests + security E2E |
| C8-T5 | Probe covers dependency CVEs via audit parsing | Met | `api/src/scripts/security-probe.ts` dependency audit + advisory parser | `PRD_CAT8/security-probe-closeout.json`, `PRD_CAT8/pnpm-audit-closeout.log` | `pnpm security:probe`, `pnpm audit --json` | dependency scan rerun |
| C8-R1 | Structured report JSON/MD with severity + repro steps | Met | `api/src/scripts/security-probe.ts` report writers | `PRD_CAT8/security-probe-closeout.json`, `PRD_CAT8/security-probe-closeout.md` | `pnpm security:probe` | n/a |
...
| C8-M3 | Manual review: rate limiting | Met | `api/src/app.ts` (`loginLimiter`, `apiLimiter`), `api/src/collaboration/index.ts` | `PRD_CAT8/manual-review-closeout.md`, `PRD_CAT8/security-probe-closeout-3.md` | request burst tests | security + API tests |
...
| C8-FIX | Fix >=2 verified vulnerabilities with before/after proof | Met | `api/src/app.ts` (rate-limit key hardening), `api/src/collaboration/index.ts` (WebSocket IP normalization), `api/src/utils/normalize-client-ip.ts`, `api/package.json` (`uuid` upgrade) | `PRD_CAT8/pnpm-audit-before.log`, `PRD_CAT8/pnpm-audit-after.log`, `PRD_CAT8/rate-limit-ip-before-after-proof.log`, `PRD_CAT8/security-probe-before.log`, `PRD_CAT8/security-probe-after-fix1.log`, `PRD_CAT8/security-probe-after.log` | `pnpm --filter @ship/api test -- src/utils/__tests__/normalize-client-ip.test.ts`, `pnpm audit --json`, `pnpm security:probe` | `pnpm type-check`, `pnpm test`, `pnpm test:coverage` |
