    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b border-border px-6 py-4">
        <div className="flex items-center gap-2.5">
          ...
          {week.is_current && (
            <span className="text-xs bg-accent/30 text-accent-foreground px-1.5 py-0.5 rounded">Current</span>
          )}
        </div>
                  ...
                  <div className="space-y-0 max-h-[300px] overflow-hidden">
                    {plan.items.map((item, i) => (
                      <div key={i} className="flex items-start gap-2.5 py-1.5">
                        <span className="text-[11px] font-semibold text-foreground/70 w-4 text-right shrink-0 mt-0.5">
                          {i + 1}.
                        </span>
            ...
            {standups.map((slot: StandupSlot) => {
              ...
              const dateLabel = (
                <div className="w-20 flex-shrink-0">
                  <span className={cn('text-xs font-medium', isToday ? 'text-accent-foreground' : isFuture ? 'text-foreground/70' : 'text-muted')}>
                    {slot.day.slice(0, 3)}
                  </span>
                  <span className={cn('text-xs ml-1', isFuture ? 'text-foreground/70' : 'text-muted')}>
              ...
              if (isFuture) {
                return (
                  <div key={slot.date} className={rowClass}>
                    {dateLabel}
                    <div className="flex-1 min-w-0">
                      <span className="text-xs text-foreground/70 italic">Upcoming</span>
                    </div>
                  </div>
                );
              }
