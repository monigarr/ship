...
1. A Render account connected to this repository.
2. Docker builds enabled (default for Render Docker services).
3. A branch with:
   - `render.yaml`
   - `Dockerfile`
   - `Dockerfile.web.prod`
...
2. Select this repository/branch and deploy.
...
`render.yaml` wires these automatically:

- `DATABASE_URL` from `ship-postgres` to `ship-api`
- `SESSION_SECRET` generated by Render
- `API_HOST` from `ship-api` host into `ship-web`
- `API_SCHEME=http` for internal web -> API proxying on Render
- `API_PORT=10000` so nginx targets the API container's listening port explicitly
- `API_HEALTH_FALLBACK=0` by default (set to `1` only if you want degraded `/api/health` and `/api/setup/status` responses when upstream is down)
- `API_STARTUP_CHECK_MODE=warn` by default (`fail` to block startup until API health check passes, `off` to disable check)
- `API_STARTUP_CHECK_PATH=/health` endpoint used for startup reachability checks
- `API_STARTUP_CHECK_RETRIES=3` and `API_STARTUP_CHECK_TIMEOUT_SECONDS=2` tune startup probe behavior

Health contract note (API <-> nginx startup check):
- API must expose unauthenticated `GET /health` that returns HTTP `200` with JSON body containing at least `{ "status": "ok" }`.
- Keep `API_STARTUP_CHECK_PATH` aligned with that endpoint. If API health semantics change, update both the API route and `ship-web` startup-check config in the same deploy.

Set these required API values in Render dashboard (**Environment** tab for `ship-api`):

- `CORS_ORIGIN` = your web origin (for example, `https://ship-web.onrender.com`)
- `APP_BASE_URL` = your API origin (for example, `https://ship-api.onrender.com`)

Optional values:

- `CDN_DOMAIN` (if file delivery should use a custom CDN domain)
- `AWS_REGION` + AWS credentials + SSM parameters (only if you want SSM-backed secrets instead of direct env vars)

### 3) Deploy and watch startup

1. Trigger deploy for all three services (or push to the tracked branch).
2. Confirm API startup logs show migration + server start.
3. Confirm web service starts nginx without config errors.

### 4) Post-deploy verification checklist

Run these checks against deployed URLs:
...
- `ship-api` sets `SKIP_SSM_SECRETS=1` in `render.yaml` so Render env vars can be used directly.
- `ship-web` proxies `/api`, `/collaboration`, and `/events` to the API service via nginx.
- Keep API and web on HTTPS origins and update `CORS_ORIGIN` whenever web hostname changes.
